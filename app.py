...
class App:
    def __init__(self):
        ...
        #instructions
        instructions = tk.Label(self.root, text="No more typing! We turn your PDF Bank Statements into Excel rows.", font="Raleway")
        instructions.grid(columnspan=colspan, column=0, row=1)
        #browse button
        self.browse_text = tk.StringVar()
        self.browse_btn = tk.Button(self.root, textvariable=self.browse_text, command=lambda:self.open_file(), font="Raleway", bg="#46bcde", fg="white", height=2, width=25)
        self.browse_text.set("Browse")
        self.browse_btn.grid(column=0, row=2)

        #change dir button
        self.cd_text = tk.StringVar()
        self.cd_btn = tk.Button(self.root, textvariable=self.cd_text, command=lambda:self.change_save_loc(), font="Raleway", bg="#46bcde", fg="white", height=2, width=25)
        self.cd_text.set("Change save location")
        self.cd_btn.grid(column=1, row=2)

        #convert button
        self.convert_text = tk.StringVar()
        self.convert_btn = tk.Button(self.root, textvariable=self.convert_text, command=lambda:self.convert_to_excel(), font="Raleway", bg="#46bcde", fg="white", height=2, width=30)
